...
def initGuesses(l, max_n):
    ...
    return [[A_initGuess, 0.5 * A_initGuess, 1.5 * A_initGuess],
            [a_initGuess, 0.001 * a_initGuess, 200 * a_initGuess],
            [infl_initGuess, 0.5 * infl_initGuess, 1.5 * infl_initGuess],
            [nu_initGuess, 0.001 * nu_initGuess, 10 * nu_initGuess]]
...
def StatModelParameters(x, y):
    ...
    minner = Minimizer(ResidualsRichards, params_richards, fcn_args=(x, y))
    ...
    results.append([fit_richards.params['A'].value, fit_richards.params['A'].stderr,
                    fit_richards.params['inv_zeta'].value, fit_richards.params['inv_zeta'].stderr,
                    fit_richards.params['l_i'].value, fit_richards.params['l_i'].stderr,
                    fit_richards.params['nu'].value, fit_richards.params['nu'].stderr,
                    add_params_ric[0], add_params_ric[1],
                    add_params_ric[2], add_params_ric[3],
                    add_params_ric[4], add_params_ric[5],
                    add_params_ric[6], add_params_ric[7],
                    MAFE_model, pvalue,
                    aics_ric, fit_richards.chisqr, fit_richards.redchi, fit_richards.bic,
                    1])
    aics = aics_ric

    # Create a Minimizer object (log)
    minner = Minimizer(ResidualsLogistic, params_logistic, fcn_args=(x, y))
    ...
    results.append([fit_logistic.params['A'].value, fit_logistic.params['A'].stderr,
                    fit_logistic.params['inv_zeta'].value, fit_logistic.params['inv_zeta'].stderr,
                    fit_logistic.params['l_i'].value, fit_logistic.params['l_i'].stderr,
                    1., 0,
                    add_params_log[0], add_params_log[1],
                    add_params_log[2], add_params_log[3],
                    add_params_log[4], add_params_log[5],
                    add_params_log[6], add_params_log[7],
                    MAFE_model, pvalue,
                    aics_log, fit_logistic.chisqr, fit_logistic.redchi, fit_logistic.bic,
                    2])
    aics = np.append(aics, aics_log)

    # Create a Minimizer object (gomp)
    minner = Minimizer(ResidualsGompertz, params_gompertz, fcn_args=(x, y))
    ...
    results.append([fit_gompertz.params['A'].value, fit_gompertz.params['A'].stderr,
                    fit_gompertz.params['inv_xi'].value, fit_gompertz.params['inv_xi'].stderr,
                    fit_gompertz.params['l_i'].value, fit_gompertz.params['l_i'].stderr,
                    ModelParameters.ZERO, 0,
                    add_params_gomp[0], add_params_gomp[1],
                    add_params_gomp[2], add_params_gomp[3],
                    add_params_gomp[4], add_params_gomp[5],
                    add_params_gomp[6], add_params_gomp[7],
                    MAFE_model, pvalue,
                    aics_gomp, fit_gompertz.chisqr, fit_gompertz.redchi, fit_gompertz.bic,
                    3])
...
def GeomModelDomains(params, modType):
    ...
    if modType != 3:
        ...
        if params['inv_xi'].stderr is not None:
...
def _valid_savgol_window(window, n_points, polyorder):
...
def CoarseDifferentiation(x, y, window):
    ...
    window = _valid_savgol_window(window, len(y), ModelParameters.POLYORDER)
...
def GetSlidingWindow(a, nu, modType):
    ...
    if denom <= 0:
        window = ModelParameters.POLYORDER + 3
    else:
        window = math.ceil((1. / denom) / ModelParameters.HOLE_RES)

    if window % 2 == 0:
        window += 1

    min_window = ModelParameters.POLYORDER + 2
    if min_window % 2 == 0:
        min_window += 1
...
def ScalingAnalysis(x, y):
    ...
    if np.any(x <= 0) or np.any(y <= 0):
        print('\n Domain contains non-positive x or y values; skipping log-log fit.\n')
        return [None]
...
def Decompose(h_pho, h_phi, ind_l_cutOff):
    ...
    if (all(h_pho[ind_l_cutOff:ModelParameters.N_SCALES] < 0) and
            all(h_phi[ind_l_cutOff:ModelParameters.N_SCALES] > 0)):
        h_plus = h_phi
        h_minus = h_pho
    elif (all(h_pho[ind_l_cutOff:ModelParameters.N_SCALES] > 0) and
          all(h_phi[ind_l_cutOff:ModelParameters.N_SCALES] < 0)):
        h_plus = h_pho
        h_minus = h_phi
    else:
...
def GroupVariants(varInfo_type, groupType):
    ...
    if groupType == 'Pathogenic(certain/confl)':
...
def MachineLearner(X_train, y_train, X_test, y_test, method='LogReg', kernel=None, chooseThreshold="f1"):
    ...
    class0_idx = np.where(model.classes_ == 0)[0][0]
    prob0_train = p_train[:, class0_idx]
    prob0_test = p_test[:, class0_idx]

    y_train_disease = (y_train == 0).astype(int)
    y_test_disease = (y_test == 0).astype(int)
    ...
    auc_train = metrics.roc_auc_score(y_train_disease, prob0_train)
    auc_test = metrics.roc_auc_score(y_test_disease, prob0_test)
    ...
    fpr, tpr, thresholds = roc_curve(y_test_disease, prob0_test)
    if chooseThreshold == "f1":
        f1_scores = [f1_score(y_test_disease, (prob0_test >= threshold).astype(int)) for threshold in thresholds]
    ...
    for j in range(ModelParameters.NUM_OF_TRAININGS):
        for k in folds:
            ...
            for train_indices, test_indices in kf.split(X, y):
                ...
                class0_idx = np.where(modelRes[-1].classes_ == 0)[0][0]
                probs_all = modelRes[-1].predict_proba(modelRes[-2].transform(X))[:, class0_idx]
                probs_split[test_indices] = probs_all[test_indices]
                ...
                if len(X_other) != 0:
                    for i in range(len(X_other)):
                        _ = modelRes[-1].predict_proba(modelRes[-2].transform(X_other[i]))[:, class0_idx]
                        probs_class_0_other[i].append(_)

                if len(X_unseen) != 0:
                    X_unseen_trans = modelRes[-2].transform(X_unseen)
                    acc_unseen = modelRes[-1].score(X_unseen_trans, y_unseen)
                    y_unseen_probs = modelRes[-1].predict_proba(X_unseen_trans)
                    prob0_unseen = y_unseen_probs[:, class0_idx]
                    y_unseen_disease = (y_unseen == 0).astype(int)

                    auc_unseen = metrics.roc_auc_score(y_unseen_disease, prob0_unseen)
                    f1_unseen = f1_score(y_unseen, modelRes[-1].predict(X_unseen_trans), average='weighted')

                    fpr, tpr, thresholds = roc_curve(y_unseen_disease, prob0_unseen)
                    f1_scores = [f1_score(y_unseen_disease, (prob0_unseen >= threshold).astype(int)) for threshold in thresholds]
                    optimal_idx = np.argmax(f1_scores)
                    optimal_threshold = thresholds[optimal_idx]
...
def Plot_CumulativeAtomNumber(data_n, data_model, min_inds_l_i, max_inds_l_i, hist_data, alpha_percentile=0.5, n_max_percentile=5, offset_xlim=5, ytitle=r'$N$ [atom]', xtitle=r'Scale index', xticks=[], statsType='median'):
    ...
    x = np.linspace(min(hist_data[3]), 5, 1000)
...
def Plot_ExponentsTraces(princ_coord, expPre, expPre_unc, PC_pre, expPost, expPost_unc, PC_post, nu, nu_unc, poreRadius, poreRadius_sd, orderOfMom=1):
    ...
    princCoord_min_PC_PD = float(princ_coord[ind_min_PC_PD][0])
...
def Plot_FeaturesImportance(princ_coord, features_importance, poreRadius, labels):
    ...
    colors_list = ["blue", "red", "green", "yellow", "magenta", "black"]
    fig, ax = plt.subplots()
    for i in range(len(labels)):
...
## Plot mutation scoring (ensemble Learning results) ##
def Plot_MutationScores(class0_probs, resIDs, n_class0, n_class1, indsAppend_LoF, indsAppend_GoF, optThres, NEUTRAL_IND=True):
    ...
    ax.set_xticks(np.arange(1, n_class + 1))
    ax.set_xticklabels(resIDs, rotation=90, fontsize=15)
